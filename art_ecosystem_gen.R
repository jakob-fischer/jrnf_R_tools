...
# Function creates a random artificial ecosystem.
#
# parameters: 
# <N>             - number of species
...
#                   using standard parameters.) Alternatively one can give a 
#                   composition created by jrnf_ae_draw_el_comp directly. Note 
#                   that this method won't add a hv-pseudospecies then but it 
#                   has to be included.
# <cat_as_lin>    - flag that indicates whether catalytical reactions 
#                   (A + C -> B + C) are treated identical to linear reactions 
#                   in terms of duplicates and in terms of the reactions of the 
#                   different types that are drawn
# <type_spec_dup> - duplicates are treated specific to the three types and not 
#                   accross them
# <rm_dup>        - Remove duplicates. Only allow one reaction for each value 
#                   combination in a column in the stoichiometric matrix. 
#                   Influenced by <type_spec_dup> and <cat_as_lin>.
# <AE_max>        - Maximum activation energy 
#
